...
if (process.env.DB_HOST)
  config.database[config.database.type].host = process.env.DB_HOST;
if (process.env.DB_NAME)
  config.database[config.database.type].database = process.env.DB_NAME;
        ...
        if (cluster.isMaster) {
          if (config.database.type === "sqlite3") {
            var db_path = path.resolve(
              ...
              config.database[config.database.type].filename
            );
