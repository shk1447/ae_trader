...
function getLineIntersect(point1, slope1, point2, slope2, key) {
  ...
  intersectX =
    (slope1 * point1.date - slope2 * point2.date + point2[key] - point1[key]) /
    (slope1 - slope2);
  // y = m1(x - x1) + y1
...
function trend(data, result, trend_type, firstValue, key) {
  if (data.length > 1) {
    ...
    var std_degree =
      (Math.atan2(Math.abs(end_y - start_y), Math.abs(end_x - start_x)) * 180) /
      Math.PI;
    ...
    if (min_idx) {
      ...
      result[trend_type + "_point"].push({
        ...
        degree: min_degree,
        high: data[min_idx].high,
        low: data[min_idx].low,
        close: _close,
        diff: diff,
