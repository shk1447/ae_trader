              ...
              for (let i = all_data.length - data.length; i < all_data.length; i++) {
                ...
                try {
                  ...
                  let bands = new BollingerBands({
                    period:20,
                    stdDev:2,
                    values:[...all_data].splice(0, i + 1).map((d) => d.close),
                  }).result

                  let obv = new OBV({
                    close:[...all_data].splice(0, i + 1).map((d) => d.close),
                    volume:[...all_data].splice(0, i + 1).map((d) => d.volume)
                  }).result;
                  
        
                  let band_data = new Array(19).concat(bands)
                  let cloud_data = new Array(77).concat(clouds)
                  let obv_data = new Array(1).concat(obv)
                  ...
                  if (metadata.length >= period) {
                    ...
                    if (prev_result) {
                      ...
                      */
                      ...
                      meta['band'] = band_data[band_data.length - 1]
                      meta['obv'] = obv_data[obv_data.length - 1];
                      ...
                      let prev_past_obv;
                      for(var c = 26; c > 0; c--) {
                        const past_obv = obv_data[obv_data.length - c];
                        if(prev_past_obv) {
                          obv_change += (past_obv - prev_past_obv)
                        }
                        ...
                        prev_past_obv = past_obv;
                      }
                      meta['future_trend'] = future_trend / 26;
                      meta['future_conversion'] = future_conversion / 26;
                      meta['future_base'] = future_base / 26;
                      meta['obv_change'] = obv_change / 26;

                      row['meta'] = JSON.stringify(meta);

                      if (insight.support > insight.resist && prev_result.insight.support < prev_result.insight.resist && meta.future_base > meta.cloud.base && prev_result.insight.resist > insight.resist && prev_result.insight.support < insight.support && prev_result.insight.support + prev_result.insight.future_resist <= prev_result.insight.resist + prev_result.insight.future_support && insight.support + insight.future_resist >= insight.future_support + insight.resist) {
                        row['marker'] = '매수';
                        ...
                        if (futures.length == 60) {
                          ...
                          var sell;
                          [...futures].forEach((d) => {
                            if(!buy && d.low > meta.future_base && d.close > meta.future_base) {
                              buy = d.close
                            }
